...
To reference different JavaScript files within NodeJS and browser based JavaScript, we can use `modules`. Specifically, ESM (**ECMAScript Module**), which allow us to structure our code and allow code to be `import`ed and outline when code can be used outside of the file/`export`ed.
...
This will allow the function to be usable outside of the file when typically applied in strict mode. An important takeaway is that this is when the code is treated as a module though.
...
Good practices will only do so much but it is best to keep them in mind about as you are developing software. It is also important to not let's structuring your code into something aesthetically pleasing to get in the way of solving a problem.
You might find issues when the code structure has reached a form of complexity which is hard to maintain. Some good practices to implement, especially when your codebase is starting to exceed 500 lines:

* No magic numbers - Don't use arbitrary indexes on arrays or values that are not intuitive to someone who isn't familiar with the codebase.

* No magic string literals - String literals which are used as part of a composition or used in multiple locations are normally candidates for formatting issues, considering string interpolation and declaring format strings.
