...
* It is common for people to add a file to staging but then to modify it after the fact. It is necessary to re-add this again if you modified it but did not make a commit which would save those changes.
...
* When you run `git commit` with a valid message associated, you may get prompted to set a username and email, these relate to authoring the of the commit. You will need to use:
