...
Generally, a data structure can be just a class or some aggregate of data. This usually relates to an (ADT) Abstract Data-Type, where we can construct a type (class, type, interface) which are some kind of abstract on top of the primitives.

However, when we talk about data structures, we are usually referring to how we can efficiently access, insert, update and/or allocate/position data.

*Why is it foundational?*

Everything you have been using such as `Array`s in JS, `List` and `Dictionary` in C# are data structures. These are powerful types that allow us to build more complex things and not bog down the computation.
