...
To make our programs interesting, we want to record and hold data within our programs. To achieve this, we use **variables**. A **variable** is a symbol within our JavaScript program that we can read and write to.
...
* Commonly `let` and `const` are used.
    ...
    * `const` does **not** allow for the variable to be reassigned.  
