...
Hypertext Transfer Protocol is the primary protocol of the web, it is built on top of **TCP/IP** which is used for establishing connections between two computers (client and server).
...
* Start Line, provide the HTTP version and method of **connection**
...
A **web-server** will need to be reachable via a URL which is typically tied to a domain name. When hosting locally, you will notice that you will use `localhost` as the domain you are accessing.
...
By default, you have heard (or not) of the root name server of `8.8.8.8` which actually points to google's root name server. This server helps with providing a list of other name servers.
